...
| [`geom_cleveland_dot()`](https://lobsterbush.github.io/tufter/reference/geom_cleveland_dot.md) | Dot plot with leader lines: position instead of length, so no zero baseline is needed |
...
for Tufte’s criteria.
[`check_contrast()`](https://lobsterbush.github.io/tufter/reference/check_contrast.md)
here overlaps GGenemy on contrast deliberately: a package that tells you
to erase ink has an obligation to check that what survives is still
visible. GGenemy goes further on accessibility, including colour-vision
simulation, and is the better tool if that is your question. The only
implementation of the data-ink ratio I could find in any language is a
Java repository, archived in 2025 and last worked on in 2010, which
requires the user to segment the image by hand before it will count
anything.
