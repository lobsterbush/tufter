...
Documentation lives at <https://lobsterbush.github.io/tufter/>. Start
with [working through a real
dataset](https://lobsterbush.github.io/tufter/articles/real-data.html),
which takes `gapminder` and `palmerpenguins` through every form and
every measurement. There’s also a [gallery on simulated
...
theme_tufte, with measured data-ink ratios of 0.22 and
0.78](reference/figures/README-before-after.png)

Default ggplot2 next to the same plot with a quartile frame and
theme_tufte, with measured data-ink ratios of 0.22 and 0.78
