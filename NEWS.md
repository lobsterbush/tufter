...
all of it changes what the package says about itself.

## The principles table and the audit disagreed

`tufte_principles()` is the package's own account of what it does, so it
mattered that it was wrong in four places.

* Two principles were named differently in the table and in the audit output,
  so the same principle appeared under two names depending on where you looked.
  The audit now uses the table's names.
* Two more were marked `audited = TRUE` when no check produced a row for them.
  "Above all else show the data" and "Layering and separation" are generative
  instructions that nothing reports on, and now say so.
* Three citations had drifted apart between the two, which happened because
  each check carried its own copy. The audit now looks the source up from the
  table, so there's only one copy to get wrong.

Two tests hold this shut from both directions: every principle marked audited
must appear in an audit of some plot, and every principle the audit names must
appear in the table with the same citation.

## Documentation

* The README's example output is generated by
  `data-raw/make_readme_output.R` rather than transcribed by hand. It had
  drifted twice, once when the range-frame fix moved the data-ink ratio and
  once when the audit messages were rewritten.

## Coverage

* `scale_fill_tufte_c()` was the one exported function with no test. It has one
  now, which brings the suite to every non-ggproto export.
